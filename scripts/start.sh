...
# --- Optional IPFS API Authentication ---
# Check if BOTH user and pass environment variables are set and non-empty
if [[ -n "$IPFS_API_USER" && -n "$IPFS_API_PASS" ]]; then
    echo "IPFS_API_USER and IPFS_API_PASS detected. Enabling API Authentication."
    echo "IPFS API Username: $IPFS_API_USER (Password is secret)"

    AUTH_SECRET="basic:$IPFS_API_USER:$IPFS_API_PASS"
    ALLOWED_PATHS_JSON='["/api/v0"]' # Protect only the v0 API path

    echo "Setting API Authorizations..."
    # Ensure the parent 'api' object exists before setting children
    ipfs config --json API.Authorizations.api "{}" || echo "API.Authorizations.api object already exists or error creating."
    ipfs config --json API.Authorizations.api.AuthSecret "\"$AUTH_SECRET\""
    ipfs config --json API.Authorizations.api.AllowedPaths "$ALLOWED_PATHS_JSON"
else
    # If one or both variables are missing, disable authentication
    echo "IPFS_API_USER and/or IPFS_API_PASS not set. Disabling API Authentication."
    # Set Authorizations to an empty object to remove any previous auth rules
    ipfs config --json API.Authorizations "{}"
    echo "API Authentication disabled."
fi
# --- End Optional API Authentication Configuration ---
